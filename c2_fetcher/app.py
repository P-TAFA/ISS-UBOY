...
def setup_environment():
    ...
    if not os.path.exists(RULES_PATH):
        default_rules = {
            ...
            "High-Risk: OpSec": ["pgp public key", "wickr", "session id", "jabber"],
            "High-Risk: Dark Logistics": ["stealth shipping", "vacuum sealed", "double mylar", "reship policy"]
        }
...
def categorize_website(text, rules):
    ...
    if category_scores:
        ...
        if "High-Risk" in primary_category:
            if hits >= 2:
                return f"🚨 {primary_category}", f"Strong match: {hits} triggers"
            else:
                return f"⚠️ Suspicious ({primary_category})", f"Weak match: {hits} trigger"
...
class AnalyserGUI:
    ...
    def _setup_results_tab(self):
        ...
        # THE SINGLE BUTTON TO ADD DATASET
    ...
    def import_dataset_file(self):
        """Allows selecting a CSV dataset via file dialog and ingests it into the database queue."""
